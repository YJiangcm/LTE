...
def knowledge_edit_template(retrieved_facts, question):
    if len(retrieved_facts) == 1:
        return "Please acknowledge the updated information provided below and respond to the subsequent question.\n\n[Updated Information]:\n" \
            + retrieved_facts[0] + "\n\n[Question]:\n" + question
        # return  f'Imagine that: {retrieved_facts[0]}\nPrompt: {question}'
    else:
        prompt = "Please acknowledge the updated information provided below and respond to the subsequent question.\n\n[Updated Information]:"
...
def compute_icl_edit_quality(
    ...
    if pre_edit:
        ...
        retrieved_facts = sentence_retrieval(prompt, hparams)
        edit_acc = icl_lm_eval(model, model_name, hparams, tok, icl_examples,
                                              target_new, knowledge_edit_template(retrieved_facts, prompt))
        ret["rewrite_acc"] = edit_acc
        ret["rewrite_retrieved_acc_1"] = float(retrieved_facts[0] == new_fact)
        ret[f"rewrite_retrieved_acc_{hparams.k}"] = float(new_fact in retrieved_facts)
        ret["rewrite_retrieved_fact"] = retrieved_facts[0]

    rephrase = record["rephrase_prompt"] if 'rephrase_prompt' in record.keys() else None
    if not pre_edit and rephrase is not None:
        retrieved_facts = sentence_retrieval(rephrase, hparams)
        rephrase_acc = icl_lm_eval(model, model_name, hparams, tok, icl_examples,
                               target_new, knowledge_edit_template(retrieved_facts, rephrase))
        ret['rephrase_acc'] = rephrase_acc
        ret["rephrase_retrieved_acc_1"] = float(retrieved_facts[0] == new_fact)
        ret[f"rephrase_retrieved_acc_{hparams.k}"] = float(new_fact in retrieved_facts)
        ret['rephrase_prompt'] = rephrase
        ret['rephrase_retrieved_fact'] = retrieved_facts[0]
    ...
    if not pre_edit and 'locality' in record.keys() and any(record['locality']):
        for locality_key in record['locality'].keys():
            ...
            retrieved_facts = sentence_retrieval(record['locality'][locality_key]['prompt'], hparams)
            post_neighbor = icl_lm_eval(model, model_name, hparams, tok, icl_examples, record['locality'][locality_key]['ground_truth'],
                                        knowledge_edit_template(retrieved_facts, record['locality'][locality_key]['prompt']), neighborhood=True)
            ...
            ret['locality'][f'{locality_key}_retrieved_acc_1'] = float(retrieved_facts[0] == new_fact)
            ret['locality'][f'{locality_key}_retrieved_acc_{hparams.k}'] = float(new_fact in retrieved_facts)
            ret['locality_prompt'] = record['locality'][locality_key]['prompt']
            ret['locality_retrieved_fact'] = retrieved_facts[0]
    ...
    if not pre_edit and 'portability' in record.keys() and any(record['portability']):
        for portability_key in record['portability'].keys():
            ...
            #                                   record['portability'][portability_key]['prompt'])
            # else:
            retrieved_facts = sentence_retrieval(record['portability'][portability_key]['prompt'], hparams)
            portability_acc = icl_lm_eval(model, model_name, hparams, tok, icl_examples, record['portability'][portability_key]['ground_truth'],
                                            knowledge_edit_template(retrieved_facts, record['portability'][portability_key]['prompt']))
            ret['portability'][f'{portability_key}_acc'] = portability_acc
            ret['portability'][f'{portability_key}_retrieved_acc_1'] = float(retrieved_facts[0] == new_fact)
            ret['portability'][f'{portability_key}_retrieved_acc_{hparams.k}'] = float(new_fact in retrieved_facts)
            ret['portability_prompt'] = record['portability'][portability_key]['prompt']
            ret['portability_retrieved_fact'] = retrieved_facts[0]
...
def compute_sent_metric(
    ...
    for key in ["inner", "outer"]:
        ...
        for i in range(value.shape[0]):
            ...
            for j in range(sep_idx): #Á¬´ø</s>Ò»¿émaskµô
                mask[i][j] = False
